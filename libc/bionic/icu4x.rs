...
pub extern "C" fn __icu4x_bionic_to_upper(ch: u32) -> u32 {
    ...
    let ret = cm.simple_uppercase(ch) as u32;
    // Once updated to the next ICU4X version (after 2.0.0-beta1),
    // this code can be simplified to use CaseMapperBorrowed (https://github.com/unicode-org/icu4x/pull/6088)
    //
    // This CaseMapper is made from compiled data, so this dtor is no-op anyway,
    // but the optimizer still links in a bunch of code
    core::mem::forget(cm);
    ret
}
...
pub extern "C" fn __icu4x_bionic_to_lower(ch: u32) -> u32 {
    ...
    let ret = cm.simple_lowercase(ch) as u32;
    // Once updated to the next ICU4X version (after 2.0.0-beta1),
    // this code can be simplified to use CaseMapperBorrowed (https://github.com/unicode-org/icu4x/pull/6088)
    //
    // This CaseMapper is made from compiled data, so this dtor is no-op anyway,
